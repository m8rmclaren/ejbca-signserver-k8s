...
createK8sOpaqueSecret "managementca" "ca.crt" "$(cat $managementca_cert | base64 | tr -d '\n')"
...
ingress_hostname="$EJBCA_INGRESS_HOSTNAME"
if [ -z "$ingress_hostname" ]; then
    echo "Using default ingress hostname ejbca.example.com"
    ingress_hostname="ejbca.example.com"
fi

ingress_secret_name="$EJBCA_INGRESS_SECRET_NAME"
if [ -z "$ingress_secret_name" ]; then
    echo "Using default secret name ejbca-ingress-tls"
    ingress_secret_name="ejbca-ingress-tls"
fi

echo "Creating server certificate for $ingress_hostname"
ejbcactl ra addendentity \
    --username "$ingress_hostname" \
    --altname dNSName="$ingress_hostname" \
    --dn "CN=$ingress_hostname" \
    --caname "Sub-CA" \
...
ejbcactl ra setclearpwd "$ingress_hostname" foo123
ejbcactl batch

server_cert="/opt/keyfactor/p12/pem/$ingress_hostname.pem"
server_key="/opt/keyfactor/p12/pem/$ingress_hostname-Key.pem"
